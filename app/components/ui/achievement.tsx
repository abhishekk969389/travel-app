...
function CountUp({ targetValue, duration = 2000 }: { targetValue: string; duration?: number }) {
  const ref = useRef<HTMLSpanElement>(null);
  const isInView = useInView(ref, { once: true, margin: "-50px" });

  const parseNumber = (val: string) => {
    const match = val.match(/^([^\d]*)([\d,.]+)(.*)$/);
    if (!match) return { prefix: "", numValue: null, suffix: val, rawNumberStr: "", hasComma: false, decimalPlaces: 0 };
    const prefix = match[1] || "";
    const rawNumberStr = match[2];
    const suffix = match[3] || "";
    const hasComma = rawNumberStr.includes(",");
    const numValue = parseFloat(rawNumberStr.replace(/,/g, ""));
    const isDecimal = rawNumberStr.includes(".") && !hasComma;
    const decimalPlaces = isDecimal ? (rawNumberStr.split(".")[1]?.length || 0) : 0;
    return { prefix, numValue, suffix, rawNumberStr, hasComma, decimalPlaces };
  };

  const parsed = parseNumber(targetValue);
  const initialText = parsed.numValue !== null ? `${parsed.prefix}0${parsed.suffix}` : targetValue;
  const [displayValue, setDisplayValue] = useState<string>(initialText);

  useEffect(() => {
    if (!isInView || parsed.numValue === null || isNaN(parsed.numValue)) return;
    ...
    const animate = (currentTime: number) => {
      ...
      const currentNum = (parsed.numValue as number) * easeOutProgress;

      let formattedNum = "";
      if (parsed.hasComma) {
        formattedNum = Math.floor(currentNum).toLocaleString();
      } else if (parsed.decimalPlaces > 0) {
        formattedNum = currentNum.toFixed(parsed.decimalPlaces);
      } else {
                ...
                return (
                  <StaggerItem key={item.id} className="relative flex flex-col items-center text-center px-2 lg:px-2 py-1 md:py-0">
                    ...
                    <div className="text-2xl sm:text-3xl md:text-2xl lg:text-2xl xl:text-3xl font-extrabold text-white tracking-tight mb-1">
                      <CountUp targetValue={item.number} />
                    </div>
